...
from logic.workers import AidaWorker, DatabaseUpdateWorker
...
class MainWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.filter_panel.setStyleSheet("#filterPanel { border: 1px solid #ccc; border-radius: 4px; }")
        filter_layout = QHBoxLayout(self.filter_panel)
        filter_layout.setContentsMargins(5, 5, 5, 5)
        self.filter_column_combo = QComboBox()
        self.filter_column_combo.addItem("Поиск по всем полям")
        self.filter_edit = QLineEdit()
        self.filter_edit.setPlaceholderText("Введите текст для поиска...")
        self.check_critical = QCheckBox("Крит. проблемы")
        self.check_upgrade = QCheckBox("Нужен апгрейд")
        self.check_no_ssd = QCheckBox("Без SSD")
        self.check_win7 = QCheckBox("Windows 7")
        self.reset_filters_btn = QPushButton("Сбросить фильтры")
        filter_layout.addWidget(QLabel("Искать в:"))
        filter_layout.addWidget(self.filter_column_combo, 1)
        filter_layout.addWidget(self.filter_edit, 2)
        filter_layout.addStretch(1)
        filter_layout.addWidget(self.check_critical)
        filter_layout.addWidget(self.check_upgrade)
        filter_layout.addWidget(self.check_no_ssd)
        filter_layout.addWidget(self.check_win7)
        separator = QFrame()
    ...
    def connect_signals(self):
        ...
        self.check_critical.stateChanged.connect(self.filter_table)
        self.check_upgrade.stateChanged.connect(self.filter_table)
        self.check_no_ssd.stateChanged.connect(self.filter_table)
        self.check_win7.stateChanged.connect(self.filter_table)
    ...
    def auto_load_data(self):
        ...
        # Фильтруем данные, оставляя только валидные записи с именем файла
        valid_data = [row for row in db_data if row.get("Имя файла")]
        
        # Заполняем кеш только валидными данными
        self.all_data.clear()
        for row_data in valid_data:
            self.all_data[row_data["Имя файла"]] = row_data
        ...
        try:
            # Сортируем валидные данные перед отображением
            sorted_data = sorted(valid_data, key=lambda x: str(x.get('Имя файла', '')))
            for row_idx, data_row in enumerate(sorted_data):
                self._populate_table_row(self.main_table, row_idx, data_row)
                self._populate_table_row(self.network_table, row_idx, data_row)
        finally:
            for table in [self.main_table, self.network_table]:
                table.setSortingEnabled(True)
                table.blockSignals(False)
                table.sortItems(table.columnCount() - 1, Qt.AscendingOrder)

        self.update_filter_combo()
        self.filter_table()

        # Отображаем корректное количество загруженных записей
        if valid_data:
            self.statusBar().showMessage(f"Загружено {len(valid_data)} записей из базы.", 5000)
        else:
            self.statusBar().showMessage("База данных пуста или не содержит валидных записей.", 5000)

        output_file = self.config.get('Settings', 'output_filename', fallback='system_analysis.xlsx')
        if os.path.exists(output_file):
            self.open_file_btn.setEnabled(True)
            self.last_file_path = output_file
    ...
    def add_table_row(self, data_row):
        ...
        for table in [self.main_table, self.network_table]:
            ...
            try:
                ...
                self._populate_table_row(table, row_pos, data_row)
            finally:
                table.blockSignals(False)
    ...
    def _populate_table_row(self, table, row_idx, data_row):
        ...
        icons = {1: style.standardIcon(style.StandardPixmap.SP_MessageBoxCritical), 2: style.standardIcon(style.StandardPixmap.SP_MessageBoxWarning), 3: style.standardIcon(style.StandardPixmap.SP_MessageBoxInformation)}
        status_icon = icons.get(category)
        for col_idx in range(table.columnCount()):
            ...
            elif col_idx == table.columnCount() - 1:
                item.setData(Qt.DisplayRole, category)
            else:
                value = str(data_row.get(header_text, ''))
                item.setText(value)
            if row_color:
    ...
    def handle_item_changed(self, item):
        ...
        self.thread = QThread()
        self.worker = DatabaseUpdateWorker(self.config, filename, header_to_update, new_value)
    ...
    def start_analysis(self):
        ...
        for w in [self.tabs, self.filter_panel, self.start_btn, self.open_file_btn]: w.setEnabled(False)
    ...
    def stop_analysis(self):
        ...
        if self.worker and hasattr(self.worker, 'is_running'):
            ...
            self.statusBar().showMessage("Остановка анализа...")

    def analysis_finished(self, output_filepath):
        # ... (здесь код не меняется)
        self.progress_bar.setVisible(False)
        for w in [self.tabs, self.filter_panel, self.start_btn]: w.setEnabled(True)
        self.stop_btn.setEnabled(False)
        for table in [self.main_table, self.network_table]:
            table.sortItems(table.columnCount() - 1, Qt.AscendingOrder)
        self.update_filter_combo()
    ...
    def update_status_bar(self, message, set_indeterminate):
        ...
        if set_indeterminate: self.progress_bar.setRange(0, 0)
    ...
    def get_filename_from_row(self, table, row):
        ...
        for i in range(table.columnCount()):
            ...
            if header_item and header_item.text() == "Имя файла":
                filename_col_index = i
                break
        if filename_col_index != -1:
            item = table.item(row, filename_col_index)
    ...
    def show_details_by_click(self, row, column):
        ...
        if filename and filename in self.all_data: self.show_details_window(filename)

    def show_details_window(self, filename):
        # ... (здесь код не меняется)
        if filename in self.details_windows:
            self.details_windows[filename].activateWindow(); self.details_windows[filename].raise_()
            return
    ...
    def show_table_context_menu(self, position):
        ...
        if filename:
            ...
            details_action.triggered.connect(lambda: self.show_details_window(filename)); menu.addSeparator()
        edit_action = menu.addAction("Изменить ячейку")
        clicked_item = active_table.itemAt(position)
        if clicked_item:
            original_flags = clicked_item.flags()
            clicked_item.setFlags(original_flags | Qt.ItemIsEditable)
        ...
        for idx in range(1, active_table.columnCount() - 1):
            header_item = active_table.horizontalHeaderItem(idx)
        ...
        if filename:
            menu.addSeparator()
            open_report_action = menu.addAction(f"Открыть отчет {filename}")
            open_report_action.triggered.connect(partial(self.open_and_select_report, filename))
    ...
    def filter_table(self):
        ...
        active_table = self.tabs.currentWidget()
        if not isinstance(active_table, QTableWidget): return
        search_text = self.filter_edit.text().lower(); search_column_name = self.filter_column_combo.currentText()
        ...
        for row in range(active_table.rowCount()):
            filename = self.get_filename_from_row(active_table, row)
            if not filename or filename not in self.all_data:
                active_table.setRowHidden(row, True); continue
            data = self.all_data[filename]; is_visible = True
            ...
            if is_visible and self.check_no_ssd.isChecked():
                has_ssd = any(k in str(data.get('Дисковые накопители', '')).lower() for k in ['ssd', 'nvme', 'snv'])
                if has_ssd: is_visible = False
    ...
    def update_filter_combo(self):
        ...
        if isinstance(active_table, QTableWidget):
            visible_headers = []
            for i in range(1, active_table.columnCount() - 1):
                header_item = active_table.horizontalHeaderItem(i)
                if header_item and not active_table.isColumnHidden(i): visible_headers.append(header_item.text())
    ...
    def reset_filters(self):
        ...
        self.check_critical.setChecked(False); self.check_upgrade.setChecked(False)
        self.check_no_ssd.setChecked(False); self.check_win7.setChecked(False)
